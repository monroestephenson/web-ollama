...
func mainEnhanced() {
	...
	cfg, showThinking := parseFlagsEnhanced()
	...
	if err := checkModelEnhanced(ollamaClient, cfg.ModelName, display); err != nil {
		os.Exit(1)
	}
	...
	for {
		...
		if cfg.AutoSearch {
			...
			if trigger.NeedsSearch {
				searchContext, sourceURLs = performSearchEnhanced(ctx, display, searxngClient, webCrawler, query, cfg)
			}
		}
...
// parseFlagsEnhanced parses command-line flags with thinking option
func parseFlagsEnhanced() (*config.Config, bool) {
...
// checkModelEnhanced verifies that the specified model exists
func checkModelEnhanced(client *ollama.Client, modelName string, display *ui.EnhancedDisplay) error {
...
// performSearchEnhanced executes web search with enhanced display
func performSearchEnhanced(ctx context.Context, display *ui.EnhancedDisplay, searxngClient *searxng.Client, webCrawler *crawler.Crawler, query string, cfg *config.Config) (string, []string) {
